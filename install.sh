...
#   curl -fsSL https://raw.githubusercontent.com/prjctimg/gtm/main/install.sh | bash -s -- --type full
#
# Environment variables:
#   VERSION        – release tag to install (default: auto-detect from installed gtmd, or latest)
#   PREFIX         – install prefix (default: $HOME/.local)
#   INSTALL_TYPE   – full | tui-only (skips interactive prompt)
#
# NOTE: chmod +x install.sh before distributing
set -euo pipefail
...
resolve_download_url() {
  ...
  if [ "$VERSION" = "latest" ]; then
    ...
    info "Latest release is a pre-release — querying API..."
...
if [ -z "$INSTALL_TYPE" ]; then
  ...
  echo ""
  echo "    1) Full      – gtm + gtmd (TUI + daemon, man pages)"
  echo "    2) TUI only  – gtm only (for existing gtmd installations)"
  echo ""

  if command -v gtmd >/dev/null 2>&1; then
    echo "  ℹ  gtmd detected on PATH — option 2 will match its version."
    echo ""
  fi
...
case "$INSTALL_TYPE" in
  full|tui-only) ;;
  *) die "Invalid install type: $INSTALL_TYPE (expected full or tui-only)" ;;
esac
...
case "$INSTALL_TYPE" in
  full)
    ...
    for f in "${TMPDIR}"/gtm.1 "${TMPDIR}"/gtmd.1; do
      [ -f "$f" ] && install_man "$f"
    done
    ok "Man pages -> ${MANDIR}/"
    ;;
